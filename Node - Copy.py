...
def treeify(listA):
	...
	for i in listA:
		for j in range(len(i)):
			if j == 0 and (i[j] not in [x.data for x in root.child]):
				useless_var = child(root,i[j])
			elif j == 0:
				#useless_var = [x for x in root.child if x.data == i[j]][0]
				useless_var = root.child[[x.data for x in root.child].index(i[j])]
			elif i[j] not in [x.data for x in useless_var.child]:
				useless_var = child(useless_var,i[j])
			else:
				useless_var = useless_var.child[[x.data for x in useless_var.child].index(i[j])]
	return root
def bottomView(root):
	listb = []
	for x in root.child:
